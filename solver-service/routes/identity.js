...
/**
 * GET /v1/identity?name=satoshi.real
 * Returns the full identity structure.
 */
router.get('/v1/identity', [nameValidator], async (req, res) => {
  ...
  try {
    const result = await bns.getIdentity(name);

    if (!result) {
      const notFound = { name: name + config.tld, error: 'Identity not found, not registered, or expired.' };
    ...
    console.error(`[identity] Error for "${name}":`, err.message);
